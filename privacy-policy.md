...
# HomeIn Privacy Policy

**Last updated:** June 26, 2026  
**Developer:** Amogh Bantwal
...
### Persistent preferences (kept until you remove them, uninstall, or clear extension storage)
...
| Selected school zones (school name, display color, and coordinates) | Remember which schools you have pinned |
| School zone boundary polygons (GeoJSON coordinates of school attendance zones) | Restore school boundaries without re-fetching from Zillow after page reloads or tab restarts |
...
| Google Maps route start and end points (coordinates and address label) | Remember the origin and destination you set via the Route button on listing cards; cleared when you manually remove them |

### Session data (automatically cleared 30 minutes after your last activity, or when you navigate to a different Zillow region)

| Data | Purpose |
|------|---------|
| Active commute zone polygon | Restore the commute overlay after a page reload without re-fetching |
| School zone overlay visibility state | Track whether school boundaries are currently rendered on the map |
...
Points of interest (fitness centers, supermarkets, hospitals, etc.) are served entirely from a database bundled within the extension itself. No external API call is made when you use the POI feature.
...
- **Zillow page content** — The extension passively intercepts Zillow's own API responses (listing coordinates and addresses, region boundary polygons, school data) as they load in the page, to avoid making duplicate network requests. Listing addresses and coordinates are held in memory and used to power the Route button that appears on each listing card, allowing you to set a listing as the start or end of a Google Maps route. This data is processed locally and not transmitted to the developer.
...
- **Persistent data** (commute destinations, school selections and their boundary polygons, POI and hazard settings, Google Maps route endpoints) is retained in your browser's local storage until you remove it within the extension, uninstall the extension, or manually clear its data via Chrome's extension settings.
- **Session data** (cached overlays, active region polygons, visibility state) is automatically cleared 30 minutes after your last interaction, or immediately when you navigate to a different Zillow region.
